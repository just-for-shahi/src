...
class SendSMS implements ShouldQueue
{
    ...
    {
        $client = new Client();
        $response = $client->post('https://api.kavenegar.com/v1/' . config('mehrino.sms.key') . '/verify/lookup.json?type=' . $type . '&receptor=' . $mobile . '&token=' . $code . '&template=' . $template);
    ...
    {
        $client = new Client();
        $url = 'https://api.kavenegar.com/v1/' . config('mehrino.sms.key') . '/sms/send.json';
