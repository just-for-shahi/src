...
import { fetchBookDetail, fetchCategories, fetchTags, fetchBooks } from "../../api";
import { StudyIcon, BookIcon } from "../../components/Icons";
import { CAT_BOOKS } from "../../Enum";
import Comment from "../../components/Comment";
import Breadcrumbs from "../../components/BreadCrump";

export default function BookDetailPage({ crumbs }) {

    const { categories, setCategories, tags, setTags, books, setBooks } = useContext(CategoryContext);
    ...
    const handleFetchData = async () => {
        setLoading(true);
        const res = await fetchBookDetail(Object.values(id));
        if (res) setData(_.get(res, "data"));
        setLoading(false);

        console.log('books-det', res)
    }
    ...
    const handleFetchCategories = async () => {
        ...
        if (res) {
            const result = _.get(res, "data").filter(r => r.type === CAT_BOOKS);
    ...
    useEffect(() => {
        ...
        if (books.length < 1) handleFetchBooks();
                            ...
                            <div className="row">
                                <div className="col-lg-8 col-md-8">
                                    ...
                                    <article className="post">
                                        <div className="post-preview"><img src={_.get(data, "cover")} alt="کتاب" className="img-fluid image" /></div>
                                            ...
                                            <div className="post-header">
                                                ...
                                                <div className="row meta-list">
                                                    <div className="col row mt-2">
                                                        ...
                                                        <div >
                                                            <span className="btn  btn-rounded  " >{_.get(data, "readers")}</span>  نفر خواننده
                                                    </div>
                                                    </div>
                                                    <div className="col row">
                                                        <BookIcon />
                                                        <span className="btn  btn-rounded  " >{_.get(data, "pages")}</span> تعداد صفحات
                                                </div>
                                                </div>
                                                <ul className="post-meta">
                                                ...
                                                {books && !subLoading &&
                                                    books.map(c =>
                                                        <li className="clearfix">
                                                            <div className="wi"><a href="#"><img src={`${process.env.PUBLIC_URL}/assets/img/blog/2.jpg`} alt="کتاب اخیر" className="img-fluid rounded" /></a></div>
