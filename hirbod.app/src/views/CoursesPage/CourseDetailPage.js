...
import { fetchCourseDetail, fetchCategories, fetchTags, fetchCourses } from "../../api";
import { StudentsIcon, ClockIcon } from "../../components/Icons";
import { CAT_COURSES } from "../../Enum";
import Comment from "../../components/Comment";
import Breadcrumbs from "../../components/BreadCrump";

export default function CourseDetailPage({ crumbs }) {

    const { categories, setCategories, tags, setTags, courses, setCourses } = useContext(CategoryContext);
    ...
    const handleFetchData = async () => {
        setLoading(true);
        const res = await fetchCourseDetail(Object.values(id));
        if (res) setData(_.get(res, "data"));
        setLoading(false);

        console.log('cur-det', res)
    }
    ...
    const handleFetchCategories = async () => {
        ...
        if (res) {
            const result = _.get(res, "data").filter(r => r.type === CAT_COURSES);
    ...
    useEffect(() => {
        ...
        if (courses.length < 1) handleFetchCourses();
                        ...
                        <div className="row align-items-center">
                            <div className="col-md-9 col-lg-7">
                                <div className="page-header-content text-white pt-4">
                                    <h1 className="text-white mb-0">دوره آموزشی</h1>
                            ...
                            <div className="row">
                                <div className="col-lg-8 col-md-8">
                                    ...
                                    <article className="post">
                                        <div className="post-preview"><img src={_.get(data, "photo")} alt="دوره" className="img-fluid image-detail" /></div>
                                        <div className="post-wrapper">
                                            <div className="post-header">
                                                ...
                                                <div className="row meta-list">
                                                    <div className="col row mt-2 ml-1">
                                                        <div className="mr-2">
                                                            <StudentsIcon />
                                                        </div>
                                                        <div >
                                                            <span className="btn ">{_.get(data, "students")}</span>  دانش آموز
                                                    </div>
                                                    </div>
                                                    <div className="col row">
                                                        <ClockIcon />
                                                        <span className="btn " >{_.get(data, "duration")}</span> مدت زمان دوره
                                                </div>
                                                </div>
                                                <ul className="post-meta">
                                                ...
                                                {courses && !subLoading &&
                                                    courses.slice(0,5).map(c =>
                                                        <li className="clearfix">
                                                            <div className="wi"><a href="#"><img src={`${process.env.PUBLIC_URL}/assets/img/blog/2.jpg`} alt="دوره اخیر" className="img-fluid rounded" /></a></div>
